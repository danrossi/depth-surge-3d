    ...
    **kwargs
    ...
    for i, frame_file in enumerate(frame_files):
        try:
            ...
            vr_frame_path = directories.get('vr_frames', output_path / INTERMEDIATE_DIRS["vr_frames"]) / f"{frame_name}.png"
            
            # Skip if already processed (resume support)
            if vr_frame_path.exists():
                callback.update_progress(f"Skipping frame {i+1}/{len(frame_files)} - Already processed", i+1, phase="extraction")
                continue
                
            # Load frame
            callback.update_progress(f"Processing frame {i+1}/{len(frame_files)} - Loading...", i+1, phase="extraction")
            ...
            vr_frame = _process_single_frame_complete(
                projector, callback, original_image, i, len(frame_files),
                directories, frame_name, super_sample_width, super_sample_height,
                apply_distortion, **kwargs
            )
...
def process_video_batch(
    ...
    **kwargs
    ...
    if super_sample_width != 1920 or super_sample_height != 1080:  # Only if different from defaults
        ...
        for i, frame_file in enumerate(frame_files):
            callback.update_progress(f"Super sampling frame {i+1}/{total_frames}", i+1, phase="super_sampling")
            _process_supersample_frame(projector, frame_file, directories, super_sample_width, super_sample_height, **kwargs)
    
    # Phase 2: Depth estimation for all frames
    callback.update_progress("Phase 2: Generating depth maps...", 0, phase="depth_estimation")
    for i, frame_file in enumerate(frame_files):
        callback.update_progress(f"Generating depth map {i+1}/{total_frames}", i+1, phase="depth_estimation")
    ...
    for i, frame_file in enumerate(frame_files):
        callback.update_progress(f"Creating stereo pair {i+1}/{total_frames}", i+1, phase="stereo_generation") 
    ...
    if apply_distortion:
        ...
        for i, frame_file in enumerate(frame_files):
            callback.update_progress(f"Fisheye distortion {i+1}/{total_frames}", i+1, phase="distortion")
    ...
    for i, frame_file in enumerate(frame_files):
        callback.update_progress(f"Assembling VR frame {i+1}/{total_frames}", i+1, phase="vr_assembly")
...
def _process_single_frame_complete(
    projector, callback, original_image, frame_idx, total_frames,
    directories, frame_name, super_sample_width, super_sample_height,
    apply_distortion, **kwargs
):
    ...
    if super_sample_width != original_image.shape[1] or super_sample_height != original_image.shape[0]:
        ...
        image = projector.apply_super_sampling(original_image, super_sample_width, super_sample_height)
    ...
    # Depth estimation
    callback.update_progress(f"Processing frame {frame_idx+1}/{total_frames} - Generating depth map...", frame_idx+1, phase="depth_estimation")
    ...
    # Stereo generation
    callback.update_progress(f"Processing frame {frame_idx+1}/{total_frames} - Creating stereo pair...", frame_idx+1, phase="stereo_generation")
    ...
    if apply_distortion:
        callback.update_progress(f"Processing frame {frame_idx+1}/{total_frames} - Fisheye projection...", frame_idx+1, phase="distortion")
    ...
    # VR assembly
    callback.update_progress(f"Processing frame {frame_idx+1}/{total_frames} - Creating VR frame...", frame_idx+1, phase="vr_assembly")
    vr_frame = projector.create_vr_format(left_final, right_final, kwargs.get('vr_format', 'side_by_side'))
...
def _process_fisheye_frame(projector, frame_file, directories, **kwargs):
    ...
    # Implementation for batch mode fisheye distortion
    pass
...
def _process_vr_assembly_frame(projector, frame_file, directories, apply_distortion, **kwargs):
    ...
    # Implementation for batch mode VR assembly
    pass 
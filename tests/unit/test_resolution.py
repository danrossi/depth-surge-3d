...
from depth_surge_3d.utils.resolution import (
    parse_custom_resolution, get_resolution_dimensions,
    calculate_vr_output_dimensions, calculate_aspect_ratio,
    classify_aspect_ratio, auto_detect_resolution
)
...
class TestResolutionUtils(unittest.TestCase):
    ...
    def test_calculate_vr_output_dimensions(self):
        ...
        # Side by side
        self.assertEqual(
            calculate_vr_output_dimensions(1920, 1080, "side_by_side"),
            (3840, 1080)
        )
        
        # Over under
        self.assertEqual(
            calculate_vr_output_dimensions(1920, 1080, "over_under"),
            (1920, 2160)
        )
